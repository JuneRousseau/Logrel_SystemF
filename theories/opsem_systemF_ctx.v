...
(** Useful lifting of pure step to step *)
...
(** --- DETERMINISM --- *)
(* To prove the determinism, we use the determinism of the semantic without
   evaluation context, and the equivalence between the semantics *)
...
Lemma fill_empty e : fill EmptyCtx e = e. Proof. done. Qed.

Lemma pure_contextual_step e1 e2 :
  e1 -h-> e2 → e1 ~> e2.
Proof. econstructor;eauto;rewrite fill_empty;reflexivity. Qed.
...
Lemma step_in_ectx K : forall e e',
  e ~> e' -> fill K e ~> fill K e'.
Proof.
  intros.
  inversion H; subst.
  rewrite! fill_comp.
...
Lemma mstep_in_ectx K : forall e e',
  e ~>* e' -> fill K e ~>* fill K e'.
Proof.
  intros. generalize dependent K.
  induction H; intros.
  + constructor.
  + apply (step_in_ectx K) in H.
    econstructor;eauto.
    firstorder.
Qed.
...
(* Possible outcomes of a pure step from K[e] *)
...
(* Possible outcomes of a step from K[e] *)
Lemma step_fill (K : ectx) e e' :
    (fill K e) ~> e' →
...
(* Possible outcomes of a mstep from K[e] *)
Lemma mstep_fill (K : ectx) e e' :
    (fill K e) ~>* e' →
    (∃ e'', e' = (fill K e'') ∧ e ~>* e'')
    ∨ (∃ v , is_val v ∧ e ~>* v ∧ (fill K v) ~>* e').
Proof.
  intros.
  apply rtc_nsteps in H.
  ...
  - inversion Hnstep as [|n' e1 e2 e3 HfillK Hnstep2];subst.
    apply step_fill in HfillK.
    destruct HfillK as [HfillK | (e'' & Hstep_e & ->)].
    + right; exists e.
      split;auto.
      split;[constructor|].
      eapply rtc_nsteps_2;eauto.
