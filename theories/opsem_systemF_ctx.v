...
| step_fst : forall v1 v2,
  is_val v1 ->
  is_val v2 ->
  (EFst (EPair v1 v2)) -h-> v1
| step_snd : forall v1 v2,
  is_val v1 ->
  is_val v2 ->
  (ESnd (EPair v1 v2)) -h-> v2
| step_lam : forall e v,
  is_val v ->
  (EApp (EAbs e) v) -h-> e.[v/]
| step_tlam : forall e,
...
Fixpoint fill (c : ectx) (e : expr) : expr :=
  match c with
  | EmptyCtx => e
  | FstCtx K => EFst (fill K e)
  | SndCtx K => ESnd (fill K e)
  | LPairCtx K t => EPair (fill K e) t
  | RPairCtx K v => EPair (of_val v) (fill K e)
  | LAppCtx K t => EApp (fill K e) t
  | RAppCtx K v => EApp (of_val v) (fill K e)
  | TAppCtx K => ETApp (fill K e)
  end.
...
(** Examples *)
Goal (EApp (EAbs (EVar 0)) EUnit) ~>* EUnit.
Proof.
  eapply star_one.
...
(* Goal (EFst *)
(*       (EApp (EAbs (EPair (EApp (EAbs EUnit) (EVar 1)) *)
(*                       (EApp (EAbs (EVar 1)) EUnit))) *)
(*          EUnit)) *)
(*   ~>* EUnit. *)
(* Proof. *)
(*   eapply star_trans with *)
(*    (EFst *)
(*       (EApp (EAbs (EPair (EApp (EAbs EUnit) (EVar 1)) *)
(*                       (EApp (EAbs (EVar 1)) EUnit))) *)
(*          EUnit).[EUnit/]). *)
...
(*             (EApp (EAbs (EPair (EApp (EAbs EUnit) (EVar 1)) *)
(*                            (EApp (EAbs (EVar 1)) EUnit))) EUnit) *)
(*   ((EPair (EApp (EAbs EUnit) (EVar 1)) *)
(*                       (EApp (EAbs (EVar 1)) EUnit)).[EUnit/])). *)
(*   simpl. done. *)
(*   1,2: simpl; done. *)
